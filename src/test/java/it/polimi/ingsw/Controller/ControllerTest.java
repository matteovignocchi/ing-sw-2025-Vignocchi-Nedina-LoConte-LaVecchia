...
class ControllerTest {
    ...
    void setUp() throws Exception {
        ...
        Controller real = new Controller(true, 1, 4, onGameEnd, loggedInUsers);
        controller = spy(real);
        doNothing().when(controller).printPlayerDashboard(any(VirtualView.class), any(Player.class), anyString());
        doNothing().when(controller).printListOfGoods(anyList(), anyString());
    }
    ...
    void testPrintAndUpdate() throws Exception {
        Controller local = new Controller(true, 1, 4, endedGames::add, loggedInUsers);
        VirtualView view = mock(VirtualView.class);
        local.addPlayer("X", view);

        Player px = local.getPlayerByNickname("X");
        local.printPlayerDashboard(view, px, "X");
        verify(view).printPlayerDashboard(any(String[][].class));
        List<Colour> goods = List.of(Colour.RED, Colour.BLUE);
        local.printListOfGoods(goods, "X");
        verify(view).printListOfGoods(anyList());
        local.updateGamePhase("X", view, GamePhase.BOARD_SETUP);
        verify(view, atLeastOnce()).updateGameState(anyString());
    }
    ...
    void testActivateDrawPhase_NormalFlow() throws Exception {
        controller.addPlayer("A", view1);
        controller.addPlayer("B", view2);
        clearInvocations(view1, view2);

        Player a = controller.getPlayerCheck("A");
        Player b = controller.getPlayerCheck("B");
        a.setConnected(true);
        b.setConnected(true);

        FlightCardBoard fbd = mock(FlightCardBoard.class);
        when(fbd.getOrderedPlayers()).thenReturn(List.of(a, b));
        setField(controller, "fBoard", fbd);

        controller.activateDrawPhase();
        verify(view1).inform(contains("You're the leader"));
        verify(view1, atLeastOnce()).updateGameState(anyString());
        verify(view2, atLeastOnce()).updateGameState(anyString());
    }
    ...
    @Test
    void testManageIfPlayerEliminatedZeroHumans() {
        Controller c = controller;
        Player p = mock(Player.class);
        when(p.getTotalHuman()).thenReturn(0);

        boolean eliminated = c.manageIfPlayerEliminated(p);

        assertTrue(eliminated);
        verify(p).setEliminated();
    }

    @Test
    void testManageIfPlayerEliminatedNonZeroHumans() {
        Controller c = controller;
        Player p = mock(Player.class);
        when(p.getTotalHuman()).thenReturn(3);

        boolean eliminated = c.manageIfPlayerEliminated(p);

        assertFalse(eliminated);
        verify(p, never()).setEliminated();
    }

    @Test
    void testAutoCommandForRemovePlayersFlagZero() throws BusinessLogicException {
        Controller c = controller;
        Player p = mock(Player.class);
        c.autoCommandForRemovePlayers(p, 0);
        verify(p, never()).getTile(anyInt(), anyInt());
    }

    @Test
    void testAutoCommandForRemovePlayersRemovesExactlyN() throws BusinessLogicException {
        Controller c = controller;
        Player p = mock(Player.class);
        HousingUnit h = mock(HousingUnit.class);
        List<Human> tokens = new ArrayList<>(List.of(Human.HUMAN, Human.HUMAN, Human.HUMAN));
        when(h.getListOfToken()).thenReturn(tokens);
        doAnswer(inv -> { tokens.remove(0); return null; }).when(h).removeHumans(0);

        when(p.getTile(anyInt(), anyInt())).thenReturn(h);

        c.autoCommandForRemovePlayers(p, 2);
        assertEquals(1, tokens.size());
    }

    @Test
    void testAutoCommandForBatteryFlagZero() throws BusinessLogicException {
        Controller c = controller;
        Player p = mock(Player.class);
        c.autoCommandForBattery(p, 0);
        verify(p, never()).getTile(anyInt(), anyInt());
    }

    @Test
    void testAutoCommandForBatteryConsumesExactlyN() throws BusinessLogicException {
        Controller c = controller;
        Player p = mock(Player.class);

        EnergyCell e = mock(EnergyCell.class);
        when(e.getCapacity()).thenReturn(3);
        when(p.getTile(anyInt(), anyInt())).thenReturn(e);
        c.autoCommandForBattery(p, 2);
        verify(e, times(2)).useBattery();
    }

    @Test
    void testAutoCommandForRemoveSingleGoodElseBranch() throws BusinessLogicException {
        Controller c = controller;
        Player p = mock(Player.class);

        StorageUnit first = mock(StorageUnit.class);
        when(first.getListOfGoods()).thenReturn(new ArrayList<>());
        StorageUnit second = mock(StorageUnit.class);
        List<Colour> goods = new ArrayList<>(List.of(Colour.GREEN, Colour.RED, Colour.BLUE));
        when(second.getListOfGoods()).thenReturn(goods);
        when(p.getTile(eq(0), eq(0))).thenReturn(first);
        when(p.getTile(eq(0), eq(1))).thenReturn(second);
        when(p.getTile(intThat(i -> !(i==0)), anyInt())).thenReturn(mock(EmptySpace.class));

        c.autoCommandForRemoveSingleGood(p, Colour.RED);

        verify(second).removeGood(1);
    }

    @Test
    void testManageGoods_AllBranches() throws Exception {
        Controller spyC = spy(controller);
        VirtualView view = mock(VirtualView.class);
        Player p = mock(Player.class);
        doReturn("P").when(spyC).getNickByPlayer(p);

        spyC.addPlayer("P", view);
        doReturn(4, 0, null).when(spyC).askPlayerIndex(p, 3);
        doReturn(false).when(spyC).askPlayerDecision(anyString(), eq(p));

        doNothing().when(spyC).addGoods(eq(p), eq(view), anyList(), eq("P"));
        doNothing().when(spyC).caseRedistribution(eq(p), eq(view), anyList(), eq("P"));
        doNothing().when(spyC).caseRemove(eq(p), eq(view), eq("P"));
        spyC.manageGoods(p, new ArrayList<>());
        verify(spyC, times(2)).inform(startsWith("SELECT:"), eq("P"));
        verify(spyC, times(1)).reportError(contains("Invalid choice"), eq("P"));
        verify(spyC, times(1)).addGoods(eq(p), eq(view), anyList(), eq("P"));
        verify(spyC, never()).caseRedistribution(any(), any(), anyList(), any());
        verify(spyC, never()).caseRemove(any(), any(), any());
    }

    @Test
    void testAddGoods_AllBranches() throws Exception {
        Controller spyC = spy(controller);
        VirtualView view = mock(VirtualView.class);
        Player p = mock(Player.class);
        doReturn("P").when(spyC).getNickByPlayer(p);
        spyC.addPlayer("P", view);
        doNothing().when(spyC).printPlayerDashboard(any(), any(), anyString());
        doNothing().when(spyC).printListOfGoods(anyList(), anyString());

        List<Colour> emptyList = new ArrayList<>();
        spyC.addGoods(p, view, emptyList, "P");
        verify(spyC).reportError("Empty list of goods", "P");
        clearInvocations(spyC);
        List<Colour> goods = new ArrayList<>(List.of(Colour.RED));
        StorageUnit su = mock(StorageUnit.class);
        when(p.getTile(2, 3)).thenReturn(su);
        when(su.isFull()).thenReturn(false);
        when(su.isAdvanced()).thenReturn(true);
        doNothing().when(su).addGood(Colour.RED);

        doReturn(new int[]{2,3}).when(spyC).askPlayerCoordinates(p);
        doReturn(null).when(spyC).askPlayerIndex(p, goods.size());
        doReturn(false).when(spyC).askPlayerDecision(anyString(), eq(p));

        spyC.addGoods(p, view, goods, "P");

        verify(su, never()).removeGood(anyInt());
        verify(su).addGood(Colour.RED);
        assertTrue(goods.isEmpty(), "La lista dei goods deve risultare vuota");

        clearInvocations(spyC);
        List<Colour> goods2 = new ArrayList<>(List.of(Colour.RED));
        StorageUnit su2 = mock(StorageUnit.class);
        when(p.getTile(2, 3)).thenReturn(su2);
        when(su2.isFull()).thenReturn(true);
        List<Colour> inside = new ArrayList<>(List.of(Colour.RED));
        when(su2.getListOfGoods()).thenReturn(inside);
        when(su2.removeGood(0)).thenReturn(Colour.RED);
        when(su2.isAdvanced()).thenReturn(false);
        doNothing().when(su2).addGood(Colour.RED);
        doReturn(null).when(spyC).askPlayerCoordinates(p);
        doReturn(null)
                .doReturn(null)
                .when(spyC).askPlayerIndex(eq(p), anyInt());
        doReturn(false).when(spyC).askPlayerDecision(anyString(), eq(p));
        spyC.addGoods(p, view, goods2, "P");
        verify(spyC).reportError("You can't place a dangerous good in a not advanced storage unit", "P");
        verify(su2).addGood(Colour.RED);
        assertEquals(1, goods2.size());
    }

    @Test
    void testCaseRedistribution_EmptyThenValidCell() throws Exception {
        VirtualView view = mock(VirtualView.class);
        Player p = mock(Player.class);
        doReturn("P").when(controller).getNickByPlayer(p);
        controller.addPlayer("P", view);
        controller.getPlayersByNickname().put("P", p);

        StorageUnit emptySu = mock(StorageUnit.class);
        when(emptySu.getListOfGoods()).thenReturn(new ArrayList<>());
        StorageUnit validSu = mock(StorageUnit.class);
        List<Colour> goods = new ArrayList<>(List.of(Colour.GREEN));
        when(validSu.getListOfGoods()).thenReturn(goods);
        doAnswer(inv -> { goods.remove(0); return Colour.GREEN; }).when(validSu).removeGood(0);

        when(p.getTile(2, 3))
                .thenReturn(emptySu)
                .thenReturn(validSu);
        doReturn(null).when(controller).askPlayerCoordinates(p);
        doReturn(null).when(controller).askPlayerIndex(p, 1);
        doNothing().when(controller).selectStorageUnitForAdd(view, p, Colour.GREEN, "P");
        doReturn(true)
                .doReturn(false)
        ...
        verify(controller).askPlayerIndex(p, 1);
        verify(validSu).removeGood(0);
        verify(controller).selectStorageUnitForAdd(view, p, Colour.GREEN, "P");
    }

    @Test
    void testCaseRedistribution_InvalidCell() throws Exception {
        VirtualView view = mock(VirtualView.class);
        Player p = mock(Player.class);
        doReturn("P").when(controller).getNickByPlayer(p);
        controller.addPlayer("P", view);
        ...
        EmptySpace es = mock(EmptySpace.class);
        when(p.getTile(anyInt(), anyInt())).thenReturn(es);
        doReturn(null).when(controller).askPlayerCoordinates(p);
    ...
    @Test
    void testSelectStorageUnitForAdd_FullThenPlaceNonRed() throws Exception {
        VirtualView view = mock(VirtualView.class);
        Player p = mock(Player.class);
        doReturn("P").when(controller).getNickByPlayer(p);
        controller.addPlayer("P", view);
        ...
        StorageUnit fullSu = mock(StorageUnit.class);
        when(fullSu.isFull()).thenReturn(true);
        StorageUnit okSu = mock(StorageUnit.class);
        when(okSu.isFull()).thenReturn(false);
    ...
    @Test
    void testSelectStorageUnitForAdd_RejectThenAcceptRed() throws Exception {
        VirtualView view = mock(VirtualView.class);
        Player p = mock(Player.class);
        doReturn("P").when(controller).getNickByPlayer(p);
        controller.addPlayer("P", view);
        controller.getPlayersByNickname().put("P", p);
        StorageUnit su1 = mock(StorageUnit.class);
        when(su1.isFull()).thenReturn(false);
        when(su1.isAdvanced()).thenReturn(false);

        StorageUnit su2 = mock(StorageUnit.class);
        when(su2.isFull()).thenReturn(false);
        when(su2.isAdvanced()).thenReturn(true);
        doNothing().when(su2).addGood(Colour.RED);
        ...
        when(p.getTile(2, 3))
                ...
                .thenReturn(su2);
        doReturn(null).when(controller).askPlayerCoordinates(p);
        controller.selectStorageUnitForAdd(view, p, Colour.RED, "P");
        verify(controller).reportError(
                "You can't place a dangerous good in a not advanced storage unit",
                "P"
        );
        verify(su2).addGood(Colour.RED);
    }
    ...
    @Test
    void testCaseRemove_WithNullCoordinate_InvokesAutoCommand() throws Exception {
        VirtualView view = mock(VirtualView.class);
        Player p = mock(Player.class);
        doReturn("P").when(controller).getNickByPlayer(p);
        controller.addPlayer("P", view);
        controller.getPlayersByNickname().put("P", p);
        doReturn(null).when(controller).askPlayerCoordinates(p);
        doNothing().when(controller).autoCommandForRemoveGoods(p,1);
        doReturn(false).when(controller).askPlayerDecision(anyString(), eq(p));
    ...
    @Test
    void testCaseRemove_WithValidStorageUnit_RemovesGoodAndPrintsDashboard() throws Exception {
        VirtualView view = mock(VirtualView.class);
        Player p = mock(Player.class);
        doReturn("P").when(controller).getNickByPlayer(p);
        controller.addPlayer("P", view);
        ...
        int[] coord = {1,2};
        doReturn(coord).when(controller).askPlayerCoordinates(p);
        StorageUnit su = mock(StorageUnit.class);
        when(p.getTile(1,2)).thenReturn(su);
        List<Colour> goods = new ArrayList<>(List.of(Colour.BLUE));
        when(su.getListOfGoods()).thenReturn(goods);
        doNothing().when(controller).printListOfGoods(goods, "P");
        doReturn(0).when(controller).askPlayerIndex(p,1);
        doReturn(false).when(controller).askPlayerDecision(anyString(), eq(p));
    ...
    @Test
    void testCaseRemove_EmptyList_ReportError() throws Exception {
        Controller ctrl = spy(new Controller(true, 1, 4, ended -> {}, new HashSet<>()));
        doReturn("P").when(ctrl).getNickByPlayer(any(Player.class));
        VirtualView view = mock(VirtualView.class);
        Player p = mock(Player.class);

        ctrl.addPlayer("P", view);
        ctrl.getPlayersByNickname().put("P", p);

        doReturn(new int[]{0,0}).when(ctrl).askPlayerCoordinates(p);
        StorageUnit emptySu = mock(StorageUnit.class);
        when(p.getTile(0,0)).thenReturn(emptySu);
        when(emptySu.getListOfGoods()).thenReturn(Collections.emptyList());
        doReturn(false).when(ctrl).askPlayerDecision(anyString(), eq(p));
        ctrl.caseRemove(p, view, "P");
        verify(ctrl).reportError("Empty list of goods", "P");
    }

    @Test
    void testCaseRemove_InvalidCell_ReportError() throws Exception {
        Controller ctrl = spy(new Controller(true, 1, 4, ended -> {}, new HashSet<>()));
        doReturn("P").when(ctrl).getNickByPlayer(any(Player.class));
        ...
        ctrl.addPlayer("P", view);
        ctrl.getPlayersByNickname().put("P", p);
        doReturn(new int[]{3,4}).when(ctrl).askPlayerCoordinates(p);
        when(p.getTile(3,4)).thenReturn(mock(EmptySpace.class));
        doReturn(false).when(ctrl).askPlayerDecision(anyString(), eq(p));
        ctrl.caseRemove(p, view, "P");
        verify(ctrl).reportError("Not valid cell", "P");
    }

    @Test
    void testAddHuman_AllBranches_Human234() throws Exception {
        Controller ctrl = spy(new Controller(true, 1, 4, ended -> {}, new HashSet<>()));
        doReturn("P").when(ctrl).getNickByPlayer(any(Player.class));
        ...
        ctrl.addPlayer("P", view);
        ctrl.getPlayersByNickname().put("P", p);
        doNothing().when(ctrl).updateGamePhase("P", view, GamePhase.CARD_EFFECT);
        List<HousingUnit> units = new ArrayList<>();
        when(p.getTile(anyInt(), anyInt())).thenAnswer(inv -> {
            HousingUnit h = mock(HousingUnit.class);
            units.add(h);
            when(h.getType()).thenReturn(Human.HUMAN);
            when(h.getTypeOfConnections()).thenReturn(Human.HUMAN234);
            doNothing().when(h).addHuman(Human.HUMAN);
            return h;
        });
        ctrl.addHuman();
        ...
        verify(p, never()).setBrownAlien();
    }


    @Test
    void testAddHuman_PurpleAndBrownAliens_Branching() throws Exception {
        Controller ctrl = spy(new Controller(true, 1, 4, ended -> {}, new HashSet<>()));
        doReturn("P").when(ctrl).getNickByPlayer(any(Player.class));
        ...
        doNothing().when(ctrl).updateGamePhase("P", view, GamePhase.CARD_EFFECT);
        when(p.presencePurpleAlien()).thenReturn(false);
        when(p.presenceBrownAlien()).thenReturn(false);
        ...
        when(p.getTile(anyInt(), anyInt())).thenAnswer(inv -> {
            ...
            units.add(h);
            when(h.getType()).thenReturn(Human.HUMAN);
            when(h.getListOfToken()).thenReturn(new ArrayList<>());
            if (i == 2 && j == 3) {
                when(h.getTypeOfConnections()).thenReturn(Human.PURPLE_ALIEN);
            } else {
                when(h.getTypeOfConnections()).thenReturn(Human.BROWN_ALIEN);
            }
            doNothing().when(h).addHuman(any());
            return h;
        });
        ...
        ctrl.addHuman();
        verify(p, never()).setPurpleAlien();
        verify(p, atLeastOnce()).setBrownAlien();
        HousingUnit purple = units.stream()
                .filter(u -> {
                    try { return u.getTypeOfConnections() == Human.PURPLE_ALIEN; }
                    catch (Exception e) { return false; }
                })
                .findFirst().orElseThrow();
        units.stream()
                .filter(u -> {
                    try { return u.getTypeOfConnections() == Human.BROWN_ALIEN; }
                    catch (Exception e) { return false; }
                })
                .forEach(h -> {
                    try { verify(h, times(1)).addHuman(Human.BROWN_ALIEN); }
                    catch (BusinessLogicException ex) { throw new RuntimeException(ex); }
                });
    }

    @Test
    void testRemoveCrewmates_numGeTotal_invokesAutoCommand() throws Exception {
        Controller spyC = spy(controller);
        Player p = mock(Player.class);
        ...
        doReturn(3).when(spyC).getNumCrew(p);
        doNothing().when(spyC).autoCommandForRemovePlayers(p, 3);
        spyC.removeCrewmates(p, 5);
        verify(spyC).autoCommandForRemovePlayers(p, 3);
    }


    @Test
    void testRemoveCrewmates_notConnectedEarly() throws Exception {
        Controller spyC = spy(controller);
        Player p = mock(Player.class);
        VirtualView v = mock(VirtualView.class);
        ...
        spyC.addPlayer("P", v);
        spyC.getPlayersByNickname().put("P", p);
        doReturn(4).when(spyC).getNumCrew(p);
        when(p.isConnected()).thenReturn(false);
        doNothing().when(spyC).autoCommandForRemovePlayers(p, 2);
        spyC.removeCrewmates(p, 2);

        verify(spyC).autoCommandForRemovePlayers(p, 2);
        verify(spyC, never()).askPlayerCoordinates(any());
    }
    ...
    void testRemoveCrewmates_invalidThenValid() throws Exception {
        ...
        int[] bad = {0,0};
        doReturn(bad, null).when(controller).askPlayerCoordinates(p);
        when(p.getTile(0,0)).thenReturn(mock(EmptySpace.class));
        HousingUnit h = mock(HousingUnit.class);
        when(p.getTile(2,3)).thenReturn(h);
        when(h.returnLenght()).thenReturn(1);
        when(h.removeHumans(0)).thenReturn(2);

        doReturn(false).when(controller).askPlayerDecision(anyString(), eq(p));

        controller.removeCrewmates(p, 1);
        verify(controller).reportError("Select a valid housing unit", "P");
        verify(h).removeHumans(0);
        verify(p).setBrownAlien();
    }

    @Test
    void testStartPlague_mixed() throws Exception {
        Player p = mock(Player.class);
        doReturn("P").when(controller).getNickByPlayer(p);
        controller.addPlayer("P", mock(VirtualView.class));
        controller.getPlayersByNickname().put("P", p);
        when(p.getTile(anyInt(), anyInt())).thenAnswer(inv -> {
            int i = inv.getArgument(0), j = inv.getArgument(1);
            HousingUnit h = mock(HousingUnit.class);

            when(h.isConnected()).thenReturn(true);
            if (i == 0 && j == 0) {
                when(h.getListOfToken())
                        .thenReturn(new LinkedList<>(List.of(Human.BROWN_ALIEN)));
                when(h.removeHumans(0)).thenReturn(2);
            }
            else if (i == 1 && j == 0) {
                when(h.getListOfToken())
                        .thenReturn(new LinkedList<>(List.of(Human.PURPLE_ALIEN)));
                when(h.removeHumans(0)).thenReturn(3);
            }
            else {
                when(h.getListOfToken())
                        .thenReturn(new LinkedList<>());
                when(h.removeHumans(0)).thenReturn(0);
            }
            return h;
        });
    ...
    @Test
    void testIsHitZone() {
        assertTrue(controller.isHitZone(0, 4));
        assertFalse(controller.isHitZone(0, 3));
        assertTrue(controller.isHitZone(1, 5));
        assertFalse(controller.isHitZone(1, 4));
        assertTrue(controller.isHitZone(2, 10));
        assertFalse(controller.isHitZone(2, 3));
        assertTrue(controller.isHitZone(3, 5));
        assertFalse(controller.isHitZone(3, 10));
        assertFalse(controller.isHitZone(4, 5));
    }

    @Test
    void testIsProtected_whenShieldFound_and_manageEnergyTrue() throws Exception {
        Controller spyC = spy(controller);
        Player p = mock(Player.class);
        VirtualView v = mock(VirtualView.class);
        doReturn("P").when(spyC).getNickByPlayer(p);
        ...
        Shield sh = mock(Shield.class);
        when(sh.getProtectedCorner(2)).thenReturn(8);
        when(p.getTile(anyInt(), anyInt())).thenAnswer(inv -> {
            int i=inv.getArgument(0), j=inv.getArgument(1);
            return (i==0&&j==0) ? sh : new EmptySpace();
        });
    ...
    @Test
    void testIsProtected_noShieldAtAll_returnsFalse() throws Exception {
        Controller spyC = spy(controller);
        Player p = mock(Player.class);
        VirtualView v = mock(VirtualView.class);
        doReturn("P").when(spyC).getNickByPlayer(p);
        ...
        when(p.getTile(anyInt(), anyInt())).thenReturn(new EmptySpace());
        boolean result = spyC.isProtected("P", 0);
        assertFalse(result);
    }

    @Test
    void testDefenceFromCannon_outOfRange() throws Exception {
        Controller spyC = spy(controller);
        Player p = mock(Player.class);
        VirtualView v = mock(VirtualView.class);
        doReturn("P").when(spyC).getNickByPlayer(p);
        doReturn(v).when(spyC).getViewCheck("P");
        spyC.addPlayer("P", v);
        spyC.getPlayersByNickname().put("P", p);
    ...
    @Test
    void testDefenceFromCannon_bigAlwaysGoToScript() throws Exception {
        Controller spyC = spy(controller);
        Player p = mock(Player.class);
        VirtualView v = mock(VirtualView.class);
        doReturn("P").when(spyC).getNickByPlayer(p);
        ...
        doReturn(false).when(spyC).isProtected("P",1);
        doReturn(true).when(spyC).scriptOfDefence("P", p, v, 6, 1);

        boolean ret = spyC.defenceFromCannon(1, true, 6, p);
        verify(spyC).scriptOfDefence("P", p, v, 6, 1);
        assertTrue(ret);
    }

    @Test
    void testDefenceFromCannon_small_notProtected_goToScript() throws Exception {
        Controller spyC = spy(controller);
        Player p = mock(Player.class);
        VirtualView v = mock(VirtualView.class);
        doReturn("P").when(spyC).getNickByPlayer(p);
        ...
        doReturn(true).when(spyC).isHitZone(2, 5);
        doReturn(false).when(spyC).isProtected("P",2);
        doReturn(false).when(spyC).scriptOfDefence("P", p, v, 5, 2);

        boolean ret = spyC.defenceFromCannon(2, false, 5, p);
        verify(spyC).scriptOfDefence("P", p, v, 5, 2);
        assertFalse(ret);
    }

    @Test
    void testDefenceFromCannon_small_protected_skipsScript() throws Exception {
        Controller spyC = spy(controller);
        Player p = mock(Player.class);
        VirtualView v = mock(VirtualView.class);
        doReturn("P").when(spyC).getNickByPlayer(p);
        ...
        doReturn(true).when(spyC).isHitZone(3, 6);
    ...
    @Test
    void testScriptOfDefence_eliminatedBranch() throws Exception {
        Controller spyC = spy(controller);
        Player p = mock(Player.class);
        VirtualView v = mock(VirtualView.class);
        doReturn("P").when(spyC).getNickByPlayer(p);

        when(p.removeFrom0(4)).thenReturn(true);
    ...
    @Test
    void testScriptOfDefence_notEliminated_hitOrSafe() throws Exception {
        Controller spyC = spy(controller);
        Player p = mock(Player.class);
        VirtualView v = mock(VirtualView.class);
        doReturn("P").when(spyC).getNickByPlayer(p);

        when(p.removeFrom1(5)).thenReturn(false);
        ...
        reset(spyC, p);
        doReturn("P").when(spyC).getNickByPlayer(p);
        when(p.removeFrom2(6)).thenReturn(true);
        doReturn(false).when(spyC).manageIfPlayerEliminated(p);
    ...
    @Test
    void testDefenceFromMeteorite_allBranches() throws Exception {
        Controller spyC = spy(controller);

        Player p0 = mock(Player.class), p1 = mock(Player.class), p2 = mock(Player.class);
        doReturn("P0").when(spyC).getNickByPlayer(p0);
        doReturn("P1").when(spyC).getNickByPlayer(p1);
        doReturn("P2").when(spyC).getNickByPlayer(p2);

        VirtualView v0 = mock(VirtualView.class),
                v1 = mock(VirtualView.class),
                v2 = mock(VirtualView.class);

        spyC.addPlayer("P0", v0);
        spyC.addPlayer("P1", v1);
        spyC.addPlayer("P2", v2);
        spyC.getPlayersByNickname().put("P0", p0);
        spyC.getPlayersByNickname().put("P1", p1);
        spyC.getPlayersByNickname().put("P2", p2);

        LinkedList<Player> players = new LinkedList<>(List.of(p0, p1, p2));
        when(p1.isConnected()).thenReturn(true);
        when(p1.isEliminated()).thenReturn(false);
        when(p2.isConnected()).thenReturn(true);
        when(p2.isEliminated()).thenReturn(false);
        doNothing().when(spyC).inform(anyString(), anyString());
        doNothing().when(spyC).printPlayerDashboard(any(), any(), anyString());
        ...
        doReturn(true).when(spyC).isHitZone(1, 6);
        doReturn(false).when(spyC).checkProtection(1, 6, "P1");
        doReturn(false).when(spyC).checkProtection(1, 6, "P2");

        spyC.defenceFromMeteorite(1, true, 6, players, 7);
        ...
        when(p1.checkNoConnector(2, 8)).thenReturn(true);
        when(p2.checkNoConnector(2, 8)).thenReturn(false);
        doReturn(true).when(spyC).isProtected("P1", 2);
        doReturn(true).when(spyC).isProtected("P2", 2);
    ...
    @Test
    void testManageEnergyCell_allBranches() throws Exception {
        Controller spyC = spy(controller);
        Player p = mock(Player.class);
        VirtualView v = mock(VirtualView.class);
        ...
        when(p.getTile(2,3)).thenReturn(ok);
        assertTrue(spyC.manageEnergyCell("P", "msg"));
        verify(ok).useBattery();
    }

    @Test
    void testCheckProtection_noShieldAtAll_returnsFalse() throws Exception {
        Player p = mock(Player.class);
        Controller ctrl = spy(controller);
        doReturn("P").when(ctrl).getNickByPlayer(p);
        ctrl.getPlayersByNickname().put("P", p);

        when(p.validityCheck(anyInt(), anyInt())).thenReturn(Status.FREE);
        when(p.getTile(anyInt(), anyInt())).thenReturn(mock(Tile.class));

        assertFalse(ctrl.checkProtection(0, 4, "P"));
        assertFalse(ctrl.checkProtection(1, 5, "P"));
        assertFalse(ctrl.checkProtection(2, 4, "P"));
        assertFalse(ctrl.checkProtection(3, 5, "P"));
        assertFalse(ctrl.checkProtection(9, 0, "P"));
    }

    @Test
    void testCheckProtection_singleCannon_controlCorners4_returnsTrue() throws Exception {
        Player p = mock(Player.class);
        Controller ctrl = spy(controller);
        doReturn("P").when(ctrl).getNickByPlayer(p);
        ctrl.getPlayersByNickname().put("P", p);

        when(p.validityCheck(eq(2), eq(1))).thenReturn(Status.USED);
        Cannon single = mock(Cannon.class);
        when(single.isDouble()).thenReturn(false);
        when(single.controlCorners(0)).thenReturn(4);
        when(p.getTile(2, 1)).thenReturn(single);

        assertTrue(ctrl.checkProtection(0, 5, "P"));
        verify(ctrl, never()).manageEnergyCell(any(), any());
    }

    @Test
    void testCheckProtection_doubleCannon_controlCorners5_invokesManageEnergyCell() throws Exception {
        Player p = mock(Player.class);
        Controller ctrl = spy(controller);
        doReturn("P").when(ctrl).getNickByPlayer(p);
        ctrl.getPlayersByNickname().put("P", p);

        when(p.validityCheck(eq(3), eq(2))).thenReturn(Status.USED);
        Cannon dbl = mock(Cannon.class);
        when(dbl.isDouble()).thenReturn(true);
        when(dbl.controlCorners(0)).thenReturn(5);
        when(p.getTile(3, 2)).thenReturn(dbl);

        doReturn(true).when(ctrl).manageEnergyCell("P", "To activate double cannon");
        assertTrue(ctrl.checkProtection(0, 6, "P"));  // col=6−4=2
        verify(ctrl).manageEnergyCell("P", "To activate double cannon");
    }

    @Test
    void testCheckProtection_fromEast_protectedBySingleCannon() throws Exception {
        Player p = mock(Player.class);
        Controller ctrl = spy(controller);
        doReturn("P").when(ctrl).getNickByPlayer(p);
        ctrl.getPlayersByNickname().put("P", p);
        ...
        int row = 7 - 5;  // row = 2
        when(p.validityCheck(eq(row), eq(6))).thenReturn(Status.USED);
        Cannon c = mock(Cannon.class);
        when(c.isDouble()).thenReturn(false);
        when(c.controlCorners(1)).thenReturn(4);
        when(p.getTile(row, 6)).thenReturn(c);

        assertTrue(ctrl.checkProtection(1, 7, "P"));
    }

    @Test
    void testCheckProtection_fromWest_doubleCannonFails() throws Exception {
        Player p = mock(Player.class);
        Controller ctrl = spy(controller);
        doReturn("P").when(ctrl).getNickByPlayer(p);
        ctrl.getPlayersByNickname().put("P", p);
        ...
        int row = 6 - 5;
        when(p.validityCheck(eq(row), eq(0))).thenReturn(Status.USED);
        Cannon dbl = mock(Cannon.class);
        when(dbl.isDouble()).thenReturn(true);
        when(dbl.controlCorners(3)).thenReturn(1);
        when(p.getTile(row, 0)).thenReturn(dbl);

        assertFalse(ctrl.checkProtection(3, 6, "P"));
    }

    @Test
    void testCheckProtection_fromSouth_columnFromSouth() throws Exception {
        Player p = mock(Player.class);
        Controller ctrl = spy(controller);
        doReturn("P").when(ctrl).getNickByPlayer(p);
        ctrl.getPlayersByNickname().put("P", p);
        when(p.validityCheck(eq(4), eq(0))).thenReturn(Status.USED);
        Cannon c = mock(Cannon.class);
        when(c.isDouble()).thenReturn(false);
        when(c.controlCorners(2)).thenReturn(4);
        when(p.getTile(4, 0)).thenReturn(c);

        assertTrue(ctrl.checkProtection(2, 5, "P"));
    }

    @Test
    void testAskStartHousingForControl_connectedLoopThenOk() throws Exception {
        Controller spyC = spy(controller);
        VirtualView view = mock(VirtualView.class);
        Player p = mock(Player.class);
        doReturn("P").when(spyC).getNickByPlayer(p);
        ...
        when(p.isConnected()).thenReturn(true);
        int[] bad = {0, 0}, good = {1, 1};
        doReturn(bad, good).when(spyC).askPlayerCoordinates(p);
        EmptySpace es = mock(EmptySpace.class);
        HousingUnit hu = mock(HousingUnit.class);
        when(hu.getType()).thenReturn(Human.HUMAN);
        when(p.getTile(0, 0)).thenReturn(es);
        when(p.getTile(1, 1)).thenReturn(hu);
        doNothing().when(spyC).reportError(anyString(), eq("P"));
        doNothing().when(spyC).inform(anyString(), eq("P"));
        doNothing().when(spyC).checkPlayerAssembly("P", 1, 1);
        spyC.askStartHousingForControl("P");
        verify(spyC).reportError("Not valid position, try again", "P");
        verify(spyC).checkPlayerAssembly("P", 1, 1);
    }

    @Test
    void testAskStartHousingForControl_disconnectedImmediate() throws Exception {
        Controller spyC = spy(controller);
        VirtualView view = mock(VirtualView.class);
        Player p = mock(Player.class);
        doReturn("P").when(spyC).getNickByPlayer(p);
        ...
        spyC.getPlayersByNickname().put("P", p);
        when(p.isConnected()).thenReturn(false);
        doNothing().when(spyC).checkPlayerAssembly("P", 2, 3);
        spyC.askStartHousingForControl("P");
        verify(spyC).checkPlayerAssembly("P", 2, 3);
    }

    @Test
    void testCheckPlayerAssembly_delegatesAndPrints() throws Exception {
        Controller spyC = spy(controller);
        VirtualView view = mock(VirtualView.class);
        Player p = mock(Player.class);
        doReturn("P").when(spyC).getNickByPlayer(p);
        spyC.addPlayer("P", view);
        spyC.getPlayersByNickname().put("P", p);
        doReturn(view).when(spyC).getViewCheck("P");
        doNothing().when(p).controlAssembly(4, 5);
        doNothing().when(spyC).printPlayerDashboard(view, p, "P");

        spyC.checkPlayerAssembly("P", 4, 5);
        verify(p).controlAssembly(4, 5);
        verify(spyC).printPlayerDashboard(view, p, "P");
    }
